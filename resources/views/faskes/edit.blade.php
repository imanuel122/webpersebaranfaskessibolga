...
<head>
    ...
    <title>Edit Faskes — {{ $faskes->nama_faskes }}</title>
...
<body class="bg-slate-50 text-slate-800">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center gap-2 text-xs text-slate-400 mb-6">
            ...
            <span>›</span>
            <a href="{{ route('faskes.show', $faskes->id) }}"
                class="hover:text-blue-600 truncate max-w-xs">{{ $faskes->nama_faskes }}</a>
            <span>›</span>
        ...
        <div class="flex items-start justify-between mb-8 gap-4">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-800">Edit Fasilitas Kesehatan</h1>
                <p class="text-slate-500 text-sm mt-1">Perbarui data untuk: <span
                        class="font-semibold text-slate-700">{{ $faskes->nama_faskes }}</span></p>
            </div>
            <a href="{{ route('faskes.show', $faskes->id) }}"
                class="flex-shrink-0 px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-500 hover:bg-slate-50 transition-colors">
                ← Batal
            </a>
        </div>
        ...
        <form action="{{ route('faskes.update', $faskes->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="section-card">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="kode_faskes"
                                    value="{{ old('kode_faskes', $faskes->kode_faskes) }}" class="form-input" />
                            </div>
                            <div>
                                <label class="form-label">Nama Faskes <span class="text-red-500">*</span></label>
                                <input type="text" name="nama_faskes"
                                    value="{{ old('nama_faskes', $faskes->nama_faskes) }}"
                                    class="form-input {{ $errors->has('nama_faskes') ? 'error' : '' }}" required />
                            </div>
                            <div>
                                ...
                                <select name="jenis_faskes_id"
                                    ...
                                    @foreach ($jenisFasilitas as $j)
                                        <option value="{{ $j->id }}"
                                            {{ old('jenis_faskes_id', $faskes->jenis_faskes_id) == $j->id ? 'selected' : '' }}>
                                            {{ $j->nama_jenis }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <input type="text" name="kategori" value="{{ old('kategori', $faskes->kategori) }}"
                                    class="form-input" />
                            </div>
                            <div>
                                ...
                                <select name="kelas_rs" class="form-input">
                                    ...
                                    @foreach (['A', 'B', 'C', 'D', 'D Pratama'] as $k)
                                        <option value="{{ $k }}"
                                            {{ old('kelas_rs', $faskes->kelas_rs) == $k ? 'selected' : '' }}>Kelas
                                            {{ $k }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <input type="text" name="akreditasi"
                                    value="{{ old('akreditasi', $faskes->akreditasi) }}" class="form-input" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-card">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="alamat" rows="2" required class="form-input {{ $errors->has('alamat') ? 'error' : '' }}">{{ old('alamat', $faskes->alamat) }}</textarea>
                            </div>
                            <div>
                                <label class="form-label">Kelurahan</label>
                                <input type="text" name="kelurahan"
                                    value="{{ old('kelurahan', $faskes->kelurahan) }}" class="form-input" />
                            </div>
                            <div>
                                ...
                                <select name="kecamatan"
                                    ...
                                    @foreach (['Sibolga Kota', 'Sibolga Selatan', 'Sibolga Utara', 'Sibolga Sambas'] as $kec)
                                        <option value="{{ $kec }}"
                                            {{ old('kecamatan', $faskes->kecamatan) == $kec ? 'selected' : '' }}>
                                            {{ $kec }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <input type="text" name="kode_pos" value="{{ old('kode_pos', $faskes->kode_pos) }}"
                                    class="form-input" placeholder="22513" />
                            </div>
                            <div>
                                <label class="form-label">Link Google Maps</label>
                                <input type="url" name="link_googlemaps"
                                    value="{{ old('link_googlemaps', $faskes->link_googlemaps) }}"
                                    class="form-input" />
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" name="latitude" id="input-latitude"
                                        ...
                                        class="form-input {{ $errors->has('latitude') ? 'error' : '' }}" required
                                ...
                                <div>
                                    ...
                                    <input type="text" name="longitude" id="input-longitude"
                                        ...
                                        class="form-input {{ $errors->has('longitude') ? 'error' : '' }}" required
                                        readonly style="background:#f8fafc;cursor:not-allowed;" />
                                </div>
                            </div>
                            <p id="coord-hint" class="text-xs text-emerald-600 mt-2 font-medium">
                                ✅ Koordinat sudah ada. Klik peta atau gunakan GPS untuk mengubah.
                            </p>
                        </div>

                        {{-- MAP --}}
                        <div id="map-picker"></div>
                        <p class="text-xs text-slate-400 mt-2 text-center">🖱 Klik peta untuk pindahkan lokasi · atau
                            📡 Gunakan tombol GPS</p>
                    </div>
                    ...
                    <div class="section-card">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="telepon" value="{{ old('telepon', $faskes->telepon) }}"
                                    class="form-input" placeholder="0631-XXXXX" />
                            </div>
                            <div>
                                <label class="form-label">Jam Operasional</label>
                                <input type="text" name="jam_operasional"
                                    value="{{ old('jam_operasional', $faskes->jam_operasional) }}" class="form-input"
                                    placeholder="24 Jam / 08.00-16.00" />
                            </div>
                            <div>
                                <label class="form-label">Kapasitas Tempat Tidur</label>
                                <input type="number" name="kapasitas_tempat_tidur"
                                    value="{{ old('kapasitas_tempat_tidur', $faskes->kapasitas_tempat_tidur ?? 0) }}"
                            ...
                            <div>
                                ...
                                <select name="status_kepemilikan" class="form-input">
                                    ...
                                    @foreach (['Pemerintah Daerah', 'Pemerintah Pusat', 'TNI/Polri', 'Swasta', 'Yayasan'] as $kep)
                                        <option value="{{ $kep }}"
                                            {{ old('status_kepemilikan', $faskes->status_kepemilikan) == $kep ? 'selected' : '' }}>
                                            {{ $kep }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="space-y-5">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <select name="status" class="form-input" required>
                                    <option value="Aktif"
                                        {{ old('status', $faskes->status) === 'Aktif' ? 'selected' : '' }}>✅ Aktif
                                    </option>
                                    <option value="Tidak Aktif"
                                        {{ old('status', $faskes->status) === 'Tidak Aktif' ? 'selected' : '' }}>❌
                                        Tidak Aktif</option>
                                </select>
                            </div>
                    ...
                    {{-- Preview koordinat --}}
                    <div class="section-card">
                        <h3 class="text-sm font-extrabold text-slate-700 mb-3">📍 Koordinat</h3>
                        <div class="bg-slate-50 rounded-xl p-3 space-y-2">
                            <div class="flex justify-between items-center">
                                <span class="text-xs text-slate-500">Latitude</span>
                                <span id="preview-lat"
                                    class="text-xs font-mono font-bold text-slate-700">{{ $faskes->latitude }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs text-slate-500">Longitude</span>
                                <span id="preview-lon"
                                    class="text-xs font-mono font-bold text-slate-700">{{ $faskes->longitude }}</span>
                            </div>
                            ...
                            <div id="preview-source" class="text-center pt-1">
                                <span
                                    class="inline-block px-2 py-0.5 rounded-full text-xs font-bold bg-blue-100 text-blue-700">📋
                                    Data Existing</span>
                            </div>
                        </div>
                        <p class="text-xs text-slate-400 mt-2 text-center">Klik peta atau GPS untuk mengubah</p>
                    </div>

                    {{-- Peringatan PostGIS --}}
                    <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                        <p class="text-xs font-bold text-amber-700 mb-1">⚠ Info PostGIS</p>
                        <p class="text-xs text-amber-600 leading-relaxed">
                            Perubahan koordinat akan otomatis update kolom
                            <code class="bg-amber-100 px-1 rounded font-mono">geom</code> via
                            <code class="bg-amber-100 px-1 rounded font-mono">ST_MakePoint</code>.
                        </p>
                    </div>
    ...
    <script>
        // ── MAP INIT ──
        const initLat = {{ $faskes->latitude ?? 1.7417 }};
        const initLng = {{ $faskes->longitude ?? 98.7792 }};
        const markerColor = '{{ $faskes->jenisFasilitas->warna_marker ?? '#2563eb' }}';
        ...
        // Marker lokasi existing
        let marker = L.marker([initLat, initLng], {
                icon: L.divIcon({
                    className: '',
                    html: `<div style="width:34px;height:34px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background:${markerColor};border:3px solid white;box-shadow:0 4px 14px rgba(0,0,0,0.3);"></div>`,
                    iconSize: [34, 34],
                    iconAnchor: [17, 34],
                })
            }).addTo(map)
            .bindPopup('<b>📍 Lokasi Saat Ini</b><br><small>Klik peta atau GPS untuk mengubah</small>')
            .openPopup();
        ...
        map.on('click', function(e) {
            updateMarker(e.latlng.lat, e.latlng.lng, 'peta');
        });

        // ── Update marker ──
        function updateMarker(lat, lng, source) {
            ...
            const label = isGPS ? '📡 Lokasi GPS Anda' : '📍 Lokasi Baru';
            ...
            // Update source badge
            const sourceDiv = document.getElementById('preview-source');
            if (isGPS) {
                sourceDiv.innerHTML =
                    '<span class="inline-block px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700">📡 Dari GPS</span>';
            } else {
                sourceDiv.innerHTML =
                    '<span class="inline-block px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-700">🖱 Diubah via Peta</span>';
            ...
            hint.textContent = isGPS ?
                ...
                '⚠ Koordinat diubah. Simpan untuk menerapkan perubahan.';
            hint.className = isGPS ?
                'text-xs text-emerald-600 mt-2 font-medium' :
                'text-xs text-amber-600 mt-2 font-medium';
        }
        ...
        function gunakanGPS() {
            ...
            btn.disabled = true;
            btnText.innerHTML = '<span class="gps-loading">⏳ Mendeteksi...</span>';
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                function(err) {
                    ...
                    showToast('error', msgs[err.code] || `GPS error: ${err.message}`);
                }, {
        ...
        <div class="bg-white border ${ok ? 'border-emerald-200' : 'border-red-200'} rounded-2xl shadow-2xl p-4 flex items-start gap-3">
            <div class="w-10 h-10 rounded-xl ${ok ? 'bg-emerald-100' : 'bg-red-100'} flex items-center justify-center flex-shrink-0 text-xl">${ok ? '✅' : '❌'}</div>
            ...
            <button onclick="this.closest('.toast-notif').remove()"
                class="flex-shrink-0 w-6 h-6 rounded-full hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 text-sm">✕</button>
        </div>`;
